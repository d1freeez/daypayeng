...
(function(global, factory) {
  ...
  var defaultOptions = {
    ...
    labelIdle: [
      'Перетащите файлы сюда или <span class="filepond--label-action">Прикрепите</span>',
      Type.STRING
    ],
  ...
  var Status = {
    EMPTY: 0,
    IDLE: 1, // waiting
    ERROR: 2, // a file is in errors state
  ...
  var createFetchFunction = function createFetchFunction() {
    ...
    // custom handler (should also handle file, load, errors, progress and abort)
  ...
  /*
                                                     function signature:
                                                       (file, metadata, load, errors, progress, abort) => {
                                                         return {
                                                         abort:() => {}
                                                       }
                                                     }
  ...
  var createFileProcessor = function createFileProcessor(processFn) {
    ...
    var process = function process(file, metadata) {
      ...
      state.request = processFn(
        ...
        // callbacks (load, errors, progress, abort)
        ...
        // errors is expected to be an object with type, code, body
        function(error) {
          // cancel updater
          state.perceivedPerformanceUpdater.clear();

          // update others about this errors
          api.fire(
            'error',
            isObject(error)
              ? error
  ...
  var createItem = function createItem() {
    ...
    var load = function load(source, loader, onload) {
      ...
      // an errors was thrown while loading the file, we need to switch to errors state
      loader.on('error', function(error) {
        setStatus(ItemStatus.LOAD_ERROR);

        fire('load-request-errors', error);
      });
    ...
    var revert = function revert(revertFileUpload, forceRevert) {
      return new Promise(function(resolve, reject) {
        ...
        revertFileUpload(
          ...
          function(error) {
            // don't set errors state when reverting is optional, it will always resolve
            if (!forceRevert) {
              resolve();
              return;
            }
  ...
  var actions = function actions(dispatch, query, state) {
    return {
      ...
      ADD_ITEM: function ADD_ITEM(_ref6) {
        ...
        item.on('load-request-errors', function(error) {
          var mainStatus = dynamicLabel(state.options.labelFileLoadError)(
            error
          );

          // is client errors, no way to recover
          ...
          // is possible server errors, so might be possible to retry
        ...
        item.on('load-file-errors', function(error) {
          dispatch('DID_THROW_ITEM_INVALID', {
            id: id,
            error: error.status,
            status: error.status
        ...
        item.on('process-errors', function(error) {
        ...
        item.on('process-revert-errors', function(error) {
      ...
      PROCESS_ITEM: getItemByQueryFromState(state, function(
        ...
        // we errors function
        item.onOnce('process-errors', function(error) {
          failure({ error: error, file: createItemAPI(item) });
          processNext();
        });
  ...
  var StateMap = {
    ...
    DID_THROW_ITEM_LOAD_ERROR: 'load-errors',
    DID_LOAD_ITEM: 'idle',
    DID_THROW_ITEM_REMOVE_ERROR: 'remove-errors',
    DID_START_ITEM_REMOVE: 'busy',
    DID_START_ITEM_PROCESSING: 'busy',
    DID_REQUEST_ITEM_PROCESSING: 'busy',
    DID_UPDATE_ITEM_PROCESS_PROGRESS: 'processing',
    DID_COMPLETE_ITEM_PROCESSING: 'processing-complete',
    DID_THROW_ITEM_PROCESSING_ERROR: 'processing-errors',
    DID_THROW_ITEM_PROCESSING_REVERT_ERROR: 'processing-revert-errors',
  ...
  var setDropEffect = function setDropEffect(dataTransfer, effect) {
    // is in try catch as IE11 will throw errors if not
    try {
  ...
  var createHopper = function createHopper(scope, validateItems, options) {
    ...
    client.allowdrop = function(items) {
      // TODO: if we can, throw errors to indicate the items cannot by dropped
  ...
  var createApp = function createApp() {
    ...
    var exposeEvent = function exposeEvent(event) {
      ...
      // if is possible errors event, make it the first param
