...
const actions = (dispatch, query, state) => ({
  ...
  ADD_ITEM: ({
    ...
    item.on('load-request-errors', error => {
    ...
    item.on('load-file-errors', error => {
      dispatch('DID_THROW_ITEM_INVALID', {
        id,
        error: error.status,
        status: error.status
    ...
    item.on('process-revert-errors', error => {
  ...
  PROCESS_ITEM: getItemByQueryFromState(state, (item, success, failure) => {
    ...
    // we errors function
    item.onOnce('process-errors', error => {
      failure({ error, file: createItemAPI(item) });
      processNext();
    });
