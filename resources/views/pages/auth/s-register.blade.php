		...
		<div class="kt-grid kt-grid--ver kt-grid--root">
			<div class="kt-grid kt-grid--hor kt-grid--root  kt-login kt-login--v1" id="kt_login">
				<div class="kt-grid__item kt-grid__item--fluid kt-grid kt-grid--desktop kt-grid--ver-desktop kt-grid--hor-tablet-and-mobile">
					...
					<!--begin::Content-->
					<div class="kt-grid__item kt-grid__item--fluid  kt-grid__item--order-tablet-and-mobile-1  kt-login__wrapper position-relative" style="background: rgba(9, 179, 239, 0.05)" >
						<!--begin::Body-->
						<div class="kt-login__body">
                            <a href="https://paydayservice.kz/" class="btn btn-primary btn-elevate kt-login__btn-primary kt-hidden-mobile position-absolute register-back"> <img src="/admin-assets/media/icons/svg/Navigation/Left-2-w.svg"/> Назад</a>

                            <!--begin::Signin-->
							<div class="kt-login__form">
								<div class="kt-login__title">
									<h3>Регистрация работника</h3>
								</div>

								<!--begin::Form-->
								<form class="kt-form" action="{{ route('register_post') }}" method="POST">
									<div class="form-group mb-3 @error('name') validated @enderror">
                                        <label for="register_name">Наименование компании <span class="kt-badge kt-badge--danger kt-badge--dot"></span></label>
										<input class="form-control @error('name') is-invalid @enderror" type="text" placeholder="ТОО Example" id="register_name" name="name" autocomplete="off" >
                                        @error('name')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
									</div>
									<div class="form-group mb-3 @error('bin') validated @enderror">
                                        <label for="register_bin">БИН <span class="kt-badge kt-badge--danger kt-badge--dot"></span></label>
										<input class="form-control @error('bin') is-invalid @enderror" type="text" placeholder="123456789101" name="bin" id="register_bin" autocomplete="off" maxlength="12" >
                                        @error('bin')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
									</div>
									<div class="form-group mb-3 @error('rg_date') validated @enderror">
                                        <label for="register_rg_date">Дата регистрации <span class="kt-badge kt-badge--danger kt-badge--dot"></span></label>
										<input class="form-control @error('rg_date') is-invalid @enderror" type="date" name="rg_date" id="register_rg_date">
                                        @error('rg_date')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
									</div>
									<div class="form-group mb-3 @error('full_name') validated @enderror">
                                        <label for="register_full_name">{{ __('table.full_name') }} <span class="kt-badge kt-badge--danger kt-badge--dot"></span></label>
										<input class="form-control @error('full_name') is-invalid @enderror" id="register_full_name" type="text" placeholder="Jhon Doe" name="full_name">
                                        @error('full_name')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
									</div>
									<div class="form-group mb-3 @error('email') validated @enderror">
                                        <label for="register_email">{{ __('table.email') }} <span class="kt-badge kt-badge--danger kt-badge--dot"></span></label>
										<input class="form-control @error('email') is-invalid @enderror" id="register_email" type="email" placeholder="example@mail.ru" name="email">
                                        @error('email')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                        <span class="form-text text-muted">После регистрация вам придет письмо на почту с потверждением.</span>
                                    </div>
									<div class="form-group mb-3 @error('email') validated @enderror">
                                        <label for="register_password">{{ __('auth.password') }} <span class="kt-badge kt-badge--danger kt-badge--dot"></span></label>
										<input class="form-control @error('email') is-invalid @enderror" id="register_password" type="password" placeholder="********" name="password">
                                        @error('password')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    <div class="form-group mb-3 @error('email') validated @enderror">
                                        <label for="register_password_confirmation">Подтверждение пароля <span class="kt-badge kt-badge--danger kt-badge--dot"></span></label>
                                        <input class="form-control @error('email') is-invalid @enderror" id="register_password_confirmation" type="password" placeholder="********" name="password_confirmation">
                                        @error('password')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    <div class="form-group mb-3">
                                        <label for="register_employees_count @error('employees_count') validated @enderror">Примерное кол-во работников</label>
                                        <input class="form-control @error('employees_count') is-invalid @enderror" id="register_employees_count" type="number" name="employees_count">
                                    ...
                                    <div class="form-group @error('accept_policy') validated @enderror">
                                        <div class="kt-checkbox-list">
                                            <label class="kt-checkbox">
                                                <input type="checkbox" name="accept_policy" class="@error('accept_policy') is-invalid @enderror">
                                                Я даю <a href="https://paydayservice.kz/main/offers/show/2" target="_blank"><b>согласие	на обработку персональных данных</b></a> и принимаю <a href="https://paydayservice.kz/main/offers/show/3" target="_blank"><b>политику конфиденциальности.</b></a> и <a href="https://paydayservice.kz/main/offers/show/1" target="_blank"><b>публичную оферту.</b></a>
                                                <span></span>
                                                @error('accept_policy')
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
