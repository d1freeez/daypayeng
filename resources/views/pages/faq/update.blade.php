        ...
        <div class="row">
            <div class="col-sm-12">
                ...
                <div class="kt-portlet">
                    ...
                    <!--begin::Form-->
                    <form class="kt-form" action="{{ route('faqs.update', $item) }}" method="post">
                                ...
                                <div class="kt-section__body row">
                                    <div class="form-group col-md-12">
                                        ...
                                        <select name="is_legal" id="for_who" class="form-control">
                                            ...
                                            <option value="0" {{ $item->is_legal == 0 ? 'selected' : '' }}>Для работников</option>
                                            <option value="1" {{ $item->is_legal == 1 ? 'selected' : '' }}>Для работадателей</option>
                                        </select>
                                    </div>
                                    ...
                                    <div class="form-group col-md-12 @error('question') validated @enderror">
                                        ...
                                        <input class="form-control @error('question') is-invalid @enderror" type="text" id="faq_question" value="{{ $item->question }}" name="question" >
                                        @error('question')
                                        <div class="valid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    <div class="form-group col-md-12 @error('answer') validated @enderror">
                                        ...
                                        <textarea name="answer" id="faq_answer" class="form-control @error('answer') is-invalid @enderror summernote" rows="30" >{!! $item->answer !!}</textarea>
                                        @error('answer')
                                        <div class="valid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
...
@section('script')
    <script>
        ...
        $('#for_who').change(function () {
            ...
            let item_parent = {{ $item->parent_id }};
            $.ajax({
                type: 'GET',
                ...
                success: function (data) {
                    for (var i = 0; i < data.length; i++) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            if (item_parent === data[i].id) {
                                faq_parent.append('<option id=faq_parent_' + data[i].id + ' value=' + data[i].id + ' selected>' + data[i].title + '</option>');
                            } else {
